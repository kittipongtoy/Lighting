...
namespace Lighting.Controllers.Frontend
{
    public class LightingController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var ProductsCategory = await _db.Product_Categorys.ToListAsync();
            var ProductsSubCategory = await _db.Product_Models.ToListAsync();
            ...
            var products = new List<Output_Product_IndexVM>();
            Products.ForEach(x =>
            {
                x.Preview_Image_Index = Path.Combine(x.Folder_Path, x.Preview_Image_Index);
                products.Add(new Output_Product_IndexVM
                {
                    ModelName = x.Model,
                    Preview_img = x.Preview_Image_Index,
                    CategoryName = ProductsCategory.FirstOrDefault(cat => cat.Id == x.Product_CategoryId)!.Name_EN,
                    SubCategory = ProductsSubCategory.FirstOrDefault(sub => sub.Id == x.Product_ModelId)!.Name_EN,
                    Type_EN = x.Type_EN,
                    Type_TH = x.Type_TH
